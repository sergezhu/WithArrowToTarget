namespace Game.Code.Utilities.Extensions
{
	...
	using System.Linq;
	using Game.Code.Utilities.MathUtils;
	using UnityEngine;
	...
	public static class VectorsExtensions
	{
		...
		{
			return new(MathCore.Abs( v.x ), MathCore.Abs( v.y ));
		}

		public static Vector2Int Sign( this Vector2Int v )
		{
			return new(MathCore.Sign( v.x ), MathCore.Sign( v.y ));
		}
		...
		{
			return new(Mathf.Clamp( v.x, -1, 1 ), Mathf.Clamp( v.y, -1, 1 ));
		}
		...
		{
			return new(Mathf.Clamp01( v.x ), Mathf.Clamp01( v.y ));
		}
